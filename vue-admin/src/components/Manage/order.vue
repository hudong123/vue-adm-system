<template>
    <Row>
        <Col span="24" class="order-block pr10">
            <bread></bread>
            <Card class="mt15">
                <div slot="title">
                    <span class="bolder">
                    ...
                    <div class="mt15">
                        <Select v-model="defaultStatus" style="width:120px; position: absolute: left:20px">
                            <Option v-for="item in statusList" :value="item.value" :key="item">{{ item.label }}</Option>
                        </Select>
                        <div class="right-block">
                            <Input class="searchInput" placeholder="电话号、昵称（全称）搜索订单"><span slot="append" class="curp">搜索</span></Input>
                            <div class="search-block">
                                ...
                                <Date-picker type="date" placeholder="选择日期" class="fr w120 ml10" v-model="enddate" :clearable="false"></Date-picker>
                                <Button type="primary" class="fr ml15"><Icon type="ios-search"></Icon>查询</Button>
                                <Button type="ghost" class="fr ml15"><Icon type="ios-cloud-download-outline"></Icon>导出Excel</Button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="order-table" v-show="!isLoading">
                    <div v-for="(item, index) in orderInfo" ref="order">
                        <Card class="mt10" :class="{primaryCard : item.statusDesc === '商家已确认' || item.statusDesc === '新订单', successCard :  item.statusDesc === '用户已评价' || item.statusDesc === '商家确认完成' || item.statusDesc === '交易成功', errorCard: item.statusDesc === '用户取消' || item.statusDesc === '商家已取消' || item.statusDesc === '用户申请退款'}">
                            <p slot="title">
                                <span>下单时间：{{format(item.createTime, 'yyyy-MM-dd HH:mm')}}</span>
                                <span class="ml25" v-if="item.responseTime">商家接单: {{format(item.responseTime, 'yyyy-MM-dd HH:mm')}}</span>
                            </p>
                            <p slot="extra">
                                <span>订单号：{{item.orderId}}</span>
                                <i class="fa fa-pencil-square-o ml5 curp" @click="addOrderNote"></i>
                            </p>
                                    ...
                                    <table cellspacing="0" cellpadding="0" border="0" width="100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="ivu-table-column"><div class="ivu-table-cell">联系人电话</div></th>
                                                <th class="ivu-table-column"><div class="ivu-table-cell">服务地址</div></th>
                                                ...
                                                <th class="ivu-table-column-center w120"><div class="ivu-table-cell">订单状态</div></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td class="ivu-table-column"><div class="ivu-table-cell"><b>{{item.contactPerson}}</b></div></td>
                                                <td class="ivu-table-column"><div class="ivu-table-cell"><b>{{item.contactPersonPhone}}</b></div></td>
                                                ...
                                                <td class="ivu-table-column"><div class="ivu-table-cell">{{quantity(item.items)}}</div></td>
                                                <td class="ivu-table-column"><div class="ivu-table-cell">￥{{item.totalPrice.toFixed(2)}}</div></td>
                                                <td class="ivu-table-column-center"><div class="ivu-table-cell" :class="{primary : item.statusDesc === '商家已确认' || item.statusDesc === '新订单', success :  item.statusDesc === '用户已评价' || item.statusDesc === '商家确认完成' || item.statusDesc === '交易成功', error: item.statusDesc === '用户取消' || item.statusDesc === '商家已取消' || item.statusDesc === '用户申请退款'}"><b>{{item.statusDesc}}</b></div></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            <div class="border-top-dashed mt10 pt10">
                                ...
                                <span class="fr">支付状态：<span :class="item.paid == 1 ? 'success' : 'error' ">{{item.paid == 1 ? '已支付' : '已退款'}}</span></span>
                            </div>
                            <!-- 退款原因 -->
                            <div class="border-top-dashed mt10 pt10" v-if="item.refundApplyReason !== '' && item.refundApplyReason">
                                退款原因：<span class="warning">{{item.refundApplyReason}}</span>
                            </div>
                            <!-- end 退款原因 -->
                            <div class="border-top-dashed mt10 pt10 dsflex" v-for="(itemList, index) in item.items">
                            ...
                            <!-- 取消原因 -->
                            <div class="border-top-dashed mt10 pt10" v-if="item.cancelReason !== '' && item.cancelReason">
                                取消原因：<span class="warning">{{item.cancelReason}}</span>
                            </div>
                            <!-- end 取消原因 -->
                            <!-- 服务人员 -->
                            <div class="border-top-dashed mt10 pt10" v-if="item.technicianName">
                            ...
                            <!-- end 服务人员 -->
                            <!-- 用户评论 -->
                            <div class="border-top-dashed mt10 pt10" v-if="item.comment">
                            ...
                            <!-- end 用户评论 -->
                            <!-- 回复内容 -->
                            <div class="border-top-dashed mt10 pt10 replySubmit" style="display: none">
